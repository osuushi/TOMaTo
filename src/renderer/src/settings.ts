export function renderSettings() {
  ...
  const activationShortcutInput = document.querySelector("#activation-shortcut") as HTMLInputElement
  ...
  const hideDockIconInput = document.querySelector("#hide-dock-icon") as HTMLInputElement
...
function updateHideDockIcon() {
  ...
  if (hideDockIconInput.checked && !window.storeGet("activationShortcut")) {
    ...
    hideDockIconInput.setCustomValidity("You must set an activation shortcut before hiding the dock icon")
